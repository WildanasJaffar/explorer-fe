...
watch(
  () => sidebar_store.is_mobile_expanded,
  (v) => {
...
const resizeHandler = () => sidebar_store.handleResize()
...
<template>
  <aside
    ...
    :style="{
      width: sidebar_store.is_mobile
        ? '80%'
        : sidebar_store.sidebar_width + 'px'
    }"
  >
    <FolderTree
  ...
  <!-- Overlay for mobile -->
  <div
    v-else-if="sidebar_store.is_mobile_expanded"
    class="absolute inset-0 z-1 bg-black/50"
    @click="sidebar_store.is_mobile_expanded = false"
  ></div>
</template>